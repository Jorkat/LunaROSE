...
//-------------------------------------------------------------------------------------------------
// Tijdelijke diagnostische logging voor F_AIACT03.
static void LogAIACT03(
	const char* stage,
	stActHead* pActDATA,
	t_AIPARAM* pEVENT,
	int distance,
	...
	const char* exceptionText = NULL)
{
	...
	if (fopen_s(&logFile, "SHO_GS_AIACT03.log", "a") != 0 || logFile == NULL)
		return;
	...
	fprintf(
		...
		"stage=%s pActDATA=%p pEVENT=%p source=%p "
		"distance=%d speed=%d cur=(%.3f, %.3f) dest=(%.3f, %.3f)",
		localTime.wYear,
		localTime.wMonth,
		localTime.wDay,
		localTime.wHour,
		...
		pActDATA,
		pEVENT,
		pEVENT != NULL ? pEVENT->m_pSourCHAR : NULL,
		distance,
		speed,
		static_cast<double>(curX),
		static_cast<double>(curY),
		static_cast<double>(posX),
		static_cast<double>(posY));

	if (exceptionText != NULL)
		fprintf(logFile, " exception=%s", exceptionText);
...
void F_AIACT03(stActHead* pActDATA, t_AIPARAM* pEVENT)
{
	...
	if (pActDATA == NULL || pEVENT == NULL || pEVENT->m_pSourCHAR == NULL)
	{
		LogAIACT03(
			"invalid pointer",
			pActDATA,
	...
	const int distance = pAct->iDistance;
	const int speed = static_cast<int>(pAct->cSpeed);
	...
	// Voorkomt AI_SysRANDOM(0), negatieve ranges en integer overflow.
	if (distance <= 0 || distance > (INT_MAX / 2))
	{
		LogAIACT03(
			"invalid distance",
			pActDATA,
	...
	try
	{
		curX = pEVENT->m_pSourCHAR->Get_CurXPOS();
		curY = pEVENT->m_pSourCHAR->Get_CurYPOS();

		const int randomRange = distance * 2;

		fPosX = curX + AI_SysRANDOM(randomRange) - distance;
		fPosY = curY + AI_SysRANDOM(randomRange) - distance;

		LogAIACT03(
			"before SetCMD_MOVE2D",
			pActDATA,
			pEVENT,
			distance,
			speed,
