...
bool classUSER::Use_InventoryITEM( t_PACKET *pPacket )
{
	...
	// Custom bag/box item
	//-------------------------------------------------------------------------------------------------
	short nBagID = USEITEM_BAG_ID(pITEM->m_nItemNo);

	if (nBagID > 0)
	{
		if (!BAG_IS_VALID(nBagID))
		{
			g_LOG.CS_ODS(
				0xffff,
				"[BAG ERROR] Invalid BagID=%d for UseItem=%d\n",
				nBagID,
				pITEM->m_nItemNo
			);
		...
		int iAvailableSlots[MAX_BAG_REWARD_SLOTS];
		int iAvailableCount = 0;

		for (int iSlot = 0;
			iSlot < MAX_BAG_REWARD_SLOTS;
			++iSlot)
		{
			int iFullItemID =
				BAG_ITEM(nBagID, iSlot);

			int iItemCount =
				BAG_ITEM_COUNT(nBagID, iSlot);

			if (iFullItemID <= 0 || iItemCount <= 0)
				continue;

			iAvailableSlots[iAvailableCount] = iSlot;
			++iAvailableCount;
		}
		...
		if (iAvailableCount <= 0)
		{
			g_LOG.CS_ODS(
				0xffff,
				"[BAG ERROR] BagID=%d has no valid rewards\n",
				nBagID
			);
		...
		int iRewardCount =
			BAG_REWARD_COUNT(nBagID);

		if (iRewardCount < 1)
			iRewardCount = 1;

		if (iRewardCount > iAvailableCount)
			iRewardCount = iAvailableCount;

		g_LOG.CS_ODS(
			0xffff,
			"[BAG OPEN] UseItem=%d BagID=%d "
			"Available=%d RewardCount=%d\n",
			pITEM->m_nItemNo,
			nBagID,
			iAvailableCount,
			iRewardCount
		...
		int iSelectedSlots[MAX_BAG_REWARD_SLOTS];
		int iSelectedCount = 0;

		for (int iReward = 0;
			iReward < iRewardCount;
			++iReward)
		{
			int iRandomPosition =
				rand() % iAvailableCount;

			int iSelectedSlot =
				iAvailableSlots[iRandomPosition];

			iSelectedSlots[iSelectedCount] =
				iSelectedSlot;

			++iSelectedCount;

			for (int iMove = iRandomPosition;
				iMove < iAvailableCount - 1;
				++iMove)
			{
				iAvailableSlots[iMove] =
					iAvailableSlots[iMove + 1];
			}

			--iAvailableCount;
		}

		bool bAllRewardsAdded = true;

		for (int i = 0; i < iSelectedCount; ++i)
		{
			int iSlot = iSelectedSlots[i];

			int iFullItemID = BAG_ITEM(nBagID, iSlot);
			int iItemCount = BAG_ITEM_COUNT(nBagID, iSlot);

			tagITEM sRewardItem;
			sRewardItem.Clear();
			sRewardItem.Init(iFullItemID);

			if (sRewardItem.IsEnableDupCNT())
			{
				sRewardItem.m_uiQuantity = iItemCount;
				this->Add_ItemNSend(sRewardItem);
			}
			else
			{
				for (int iAmount = 0;
					...
					++iAmount)
				{
					tagITEM sSingleItem;
					sSingleItem.Clear();
					sSingleItem.Init(iFullItemID);
					sSingleItem.m_uiQuantity = 1;

					this->Add_ItemNSend(sSingleItem);
				}
			}

			g_LOG.CS_ODS(
				0xffff,
				"[BAG GIVE] BagID=%d Slot=%d Type=%d ItemNo=%d Count=%d\n",
				nBagID,
				iSlot + 1,
				sRewardItem.GetTYPE(),
				sRewardItem.GetItemNO(),
				iItemCount
			);
		}

		// Resultaatpacket naar de client
		classPACKET* pCPacket = Packet_AllocNLock();

		if (pCPacket)
		{
			pCPacket->m_HEADER.m_wType = GSV_BAG_RESULT;
			pCPacket->m_HEADER.m_nSize = sizeof(gsv_BAG_RESULT);

			pCPacket->m_gsv_BAG_RESULT.m_btRewardCount =
				(BYTE)iSelectedCount;

			for (int i = 0; i < MAX_BAG_RESULT_ITEMS; ++i)
			{
				pCPacket->m_gsv_BAG_RESULT
					.m_Rewards[i]
					.m_iFullItemID = 0;
