...
//-------------------------------------------------------------------------------------------------
static void LogMoveBadAllocation(
	CObjAI* pObject,
	const char* stage,
	float speed)
{
	...
	if (fopen_s(&logFile, "SHO_GS_MOVE_BAD_ALLOC.log", "a") != 0 || logFile == NULL)
		return;
	...
	fprintf(
		...
		"stage=%s object=%p name=%s speed=%.3f "
		"cur=(%.3f, %.3f) goto=(%.3f, %.3f) "
		"workingSetMB=%.2f privateMB=%.2f\n",
		localTime.wYear,
		localTime.wMonth,
		localTime.wDay,
		localTime.wHour,
		...
		pObject != NULL ? static_cast<double>(pObject->m_PosCUR.x) : 0.0,
		pObject != NULL ? static_cast<double>(pObject->m_PosCUR.y) : 0.0,
		pObject != NULL ? static_cast<double>(pObject->m_PosGOTO.x) : 0.0,
		pObject != NULL ? static_cast<double>(pObject->m_PosGOTO.y) : 0.0,
...
void CObjAI::Start_MOVE(float fSpeed)
{
	...
	if (Get_STATE() != CS_MOVE)
	{
		...
		try
		{
			const short moveAnimation = this->GetANI_Move();

			this->Set_MOTION(
				moveAnimation,
				m_fCurMoveSpeed,
				this->Get_MoveAniSPEED());
		}
		catch (const std::bad_alloc&)
		{
			LogMoveBadAllocation(this, "Start_MOVE Set_MOTION std::bad_alloc", fSpeed);

			// Stop this object cleanly instead of allowing the exception
			// to terminate the complete zone thread or game server.
			m_wCommand = CMD_STOP;
			m_wState = CS_STOP;
