...
//-------------------------------------------------------------------------------------------------
static void LogZoneBadAllocation(
	CZoneTHREAD* pZone,
	CGameOBJ* pObject,
	const char* stage)
{
	...
	if (fopen_s(&logFile, "SHO_GS_BAD_ALLOC.log", "a") != 0 || logFile == NULL)
		return;
		...
		sizeof(memoryInfo)))
	{
		...
		privateBytes = memoryInfo.PrivateUsage;
	}

	fprintf(
		logFile,
		"[%04u-%02u-%02u %02u:%02u:%02u.%03u] "
		"stage=%s zone=%d object=%p index=%d type=%d isUser=%d "
		"name=%s workingSetMB=%.2f privateMB=%.2f\n",
		localTime.wYear,
		localTime.wMonth,
		localTime.wDay,
		localTime.wHour,
		...
		pZone != NULL ? pZone->Get_ZoneNO() : -1,
		pObject,
		pObject != NULL ? pObject->Get_INDEX() : -1,
		pObject != NULL ? pObject->Get_TYPE() : -1,
		pObject != NULL && pObject->IsUSER() ? 1 : 0,
		pObject != NULL && pObject->Get_NAME() != NULL
		? pObject->Get_NAME()
		: "(null)",
		static_cast<double>(workingSet) / (1024.0 * 1024.0),
		static_cast<double>(privateBytes) / (1024.0 * 1024.0));
...
void CZoneTHREAD::Execute()
{
	...
	while (!this->Terminated) {
		...
		while (pObjNODE) {
			...
			try {
				procSucceeded = pObjNODE->DATA->Proc();
			}
			catch (const std::bad_alloc&) {
				pDelOBJ = pObjNODE->DATA;
				LogZoneBadAllocation(this, pDelOBJ, "CGameOBJ::Proc std::bad_alloc");

				// A bad allocation means the process is under serious memory pressure.
				// Remove the failing non-user object so it cannot throw every frame.
				if (pDelOBJ != NULL && !pDelOBJ->IsUSER()) {
					this->Sub_DIRECT(pDelOBJ);
					SAFE_DELETE(pDelOBJ);
					objectAlive = false;
				}
				else {
					// Do not delete a user object directly from this catch block.
					// Let the normal disconnect path handle it on the next iteration.
					procSucceeded = false;
				}
			}
			catch (...) {
				pDelOBJ = pObjNODE->DATA;
				LogZoneBadAllocation(this, pDelOBJ, "CGameOBJ::Proc unknown exception");
				throw;
			}

			if (objectAlive && !procSucceeded) {
				pDelOBJ = pObjNODE->DATA;
