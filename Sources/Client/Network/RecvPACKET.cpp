...
void CRecvPACKET::Recv_gsv_BAG_RESULT()
{
	BYTE btRewardCount =
		m_pRecvPacket->m_gsv_BAG_RESULT.m_btRewardCount;

	if (btRewardCount <= 0)
		return;

	if (btRewardCount > MAX_BAG_RESULT_ITEMS)
		btRewardCount = MAX_BAG_RESULT_ITEMS;

	CreateMsgBoxData MsgBoxData;

	MsgBoxData.strMsg = "You received:";
	MsgBoxData.bModal = true;
	MsgBoxData.iButtonType = CMsgBox::BT_OK;
	MsgBoxData.iInvokerDlgID = 0;
	MsgBoxData.iMsgType = 0;
	MsgBoxData.pCmdOk = NULL;
	MsgBoxData.pCmdCancel = NULL;

	for (int i = 0; i < btRewardCount; ++i)
	{
		int iFullItemID =
			m_pRecvPacket->m_gsv_BAG_RESULT
			.m_Rewards[i]
			.m_iFullItemID;
		...
		if (iFullItemID <= 0 || iQuantity <= 0)
			continue;

		tagITEM RewardItem;
		RewardItem.Clear();
		RewardItem.Init(iFullItemID);

		if (RewardItem.IsEmpty())
			continue;

		if (RewardItem.IsEnableDupCNT())
			RewardItem.m_uiQuantity = iQuantity;
		else
			RewardItem.m_uiQuantity = 1;

		CIconItem* pItemIcon =
			new CIconItem(&RewardItem);

		MsgBoxData.m_Icons.push_back(pItemIcon);
	}

	if (MsgBoxData.m_Icons.empty())
		return;
