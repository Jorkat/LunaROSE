...
void CUpgradeDlgStateResult::Update( POINT ptMouse )
{
	...
	const DWORD dwCurrTime = g_GameDATA.GetGameTime();

	const int iUpdateResult =
		m_pResultGuage->Update(
			ptMouse,
			m_dwPrevTime,
			dwCurrTime);

	// Vanaf nu meet de gauge alleen de verstreken tijd
	// sinds de vorige frame-update.
	m_dwPrevTime = dwCurrTime;

	if (iUpdateResult == CGuage::UPDATE_END)
	{
