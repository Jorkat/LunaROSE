...
int		CGuage::Update( POINT ptMouse, DWORD dwPrevTime, DWORD dwCurrTime )
{
	...
	const DWORD dwUpdateSpace = dwCurrTime - dwPrevTime;

	const unsigned __int64 ullProgress =
		static_cast<unsigned __int64>(m_dwAutoIncrementRemainder) +
		static_cast<unsigned __int64>(m_iAutoIncrementValue) *
		static_cast<unsigned __int64>(dwUpdateSpace);

	const int iIncrement =
		static_cast<int>(ullProgress / 1000);

	m_dwAutoIncrementRemainder =
		static_cast<DWORD>(ullProgress % 1000);
